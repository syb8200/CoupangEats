...
- 개발 도중 발생한 이슈   
: Home - API 연동 과정에서 데이터가 들어오지 않는 어려움 겪음 / API 이미지 관련 이슈 발생   
: 해결) 명세서에 있는대로 data class 이름을 설정해줌으로써 해결, 이미지 관련 이슈 부분은 아직 서버 쪽과 상의 중   
   
- 위클리스크럼   
<진행상황>   
: 도나 - 총 16개 API 구성. 18번의 구성이 상당히 복잡해 애를 먹는 중. 오늘 중에 18번만 하지 않을까함   
: 코어 - 5개 API 구성. 현재 26번 명세서를 작성중   
: 노바 - Home 음식점 API 연동 진행중, Store 세부 정보 UI 구현 진행중   
<목표>   
: 도나 - 2차 피드백 전까지 18, 19번을 포함한 20개의 API 달성하는것이 목표 / 구현 API에 대해 validation 처리 확인   
: 코어 - 2차 피드백 전까지 최소 Favorite 도메인까지 API를 작성하는 것이 목표   
: 노바 - 2차 피드백 전까지 Home 음식점 API 연동 완료 / Store 세부정보 UI 구현 완료   
<문제점 공유 (클라이언트-서버)>   
: 도나 - 18번 메뉴 담기 API 구성에 대한 의견 공유 / 실제 서비스에서는 상황에 따라 메뉴 담기의 기능에 기존 카트 삭제 기능이 포함되기도 하기때문에 클라이언트 쪽의 행위(ex-버튼을 누른다)에 여러개의 API를 묶는 것으로 결정   
: 코어 - 점: 유저 주소 정보 추가 API를 만드는 중 위치 기반 서비스가 적용되는 부분이 있어서 여기에 외부 API가 적용이 되는 건지 궁금해서 클라이언트와 논의 / 논의 결과 외부 API가 적용되는 것으로 확인    
: 노바 - 음식점 리스트 외에 전반적인 이미지 처리 관련 문제 논의 / imgbb 링크로 이미지 받고, 해당 이미지 유효기간을 늘리는 방법으로 해결 (링크 문제 발생으로 다른 방법 논의 중)   
...
- 개발 도중 발생한 이슈   
: Store - 상단 ToolBar 위치에 따른 아이콘 색 변경에 어려움 겪음   
: 해결) 아직 방법을 찾지 못해 피드백 때 질문 요청 예정   
   
- 2차 피드백   
: 전체적으로 API 연동 개수가 부족해보이며, 생산성이 떨어지는 것은 사실이나, 유사도는 높아보인다는 평가 받음   
: 우선순위에 맞춰서 계획한 것을 다 끝내면 수료할 가능성 높아 보임   
: 이미지 관련 이슈는 서버 측에서 피드백 시간에 질문을 함으로써 해결, 이 부분은 빠른 시일 내로 처리해주겠다고 약속함   
: Store 상단 툴바 관련 질문에 대한 피드백 = 따로 툴 바를 만들어서 스크롤 위치에 따라 만들어도 될 것 같음   
: 사소한 디테일 (ripple 등..) 같은 경우는 따로 몰아서 진행할 예정   
...
- 개발 도중 발생한 이슈   
: Store - 상단 ToolBar 위치에 따른 아이콘 색 변경에 어려움 겪음 / 많은 데이터 받아오는 경우 대비하여 코드 관련 어려움 겪음   
: 해결) 새로운 툴 바 생성하여 구현 완료 / 코드 구조를 수정하여 구현 완료   
...
- 개발 도중 발생한 이슈   
: Store - 클릭한 아이템 별로 데이터를 받아올 때, 비동기식 처리 방식으로 인해 어려움을 겪음   
: 해결) 코드 수정 및 response 부분에 변수 선언하여 해결   
...
- 개발 도중 발생한 이슈   
: Store & Menu - API로부터 받아온 Id 값을 intent로 전달하는 과정에서, Activity 생명주기로 인해 없어지는 이슈 발생   
: 해결) sharedPreference를 활용해여 해결   
